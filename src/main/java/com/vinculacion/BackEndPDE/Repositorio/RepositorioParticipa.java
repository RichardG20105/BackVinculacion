...
public interface RepositorioParticipa extends JpaRepository<Participa, Long>{
	...
	@Query(value = "SELECT * FROM participa WHERE iddocente = ?1 and extract(year from participacioninicio) = ?2", nativeQuery = true)
	...
	Boolean existsByDocenteAndProyectoAndCargo(Docente docente, Proyecto proyecto, String cargo);
}
